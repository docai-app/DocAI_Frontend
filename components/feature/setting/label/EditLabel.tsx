...
export default function EditLabel(props: EditLabelProps) {
    ...
    const isContain = (value: any) => {
        const index = _.findIndex(tag?.functions, function (func: any) { return func.id == value });
        return index == -1
    }
                                            ...
                                            <div className="w-full flex flex-row mt-2">
                                                ...
                                                <div className="flex flex-col justify-start items-start w-3/4">
                                                    {tagTypes?.functions?.map((item: any, index: number) => {
                                                        return (
                                                            <div key={index}>
                                                                <input type={'checkbox'} name={item.title} defaultChecked={!isContain(item.id)}
                                                                    onChange={(e) => {
                                                                        if (e.target.checked) {
                                                                            updateTagFunctionsHandler(tag.id, item.id);
                                                                        } else {
                                                                            deleteTagFunctionsHandler(tag.id, item.id);
                                                                        }
