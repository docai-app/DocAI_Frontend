...
export default function ScheamDataContainer() {
    ...
    const [{ data: getSmartExtractionSchemasData, loading: loading }, getSmartExtractionSchemas] =
        useAxios(apiSetting.SmartExtractionSchemas.getSmartExtractionSchemas(has_label, page), { manual: true });
    ...
    useEffect(() => {
        ...
        getSmartExtractionSchemas(apiSetting.SmartExtractionSchemas.getSmartExtractionSchemas(has_label, page))
    ...
    useEffect(() => {
        if (getSmartExtractionSchemasData && getSmartExtractionSchemasData.success) {
            ...
            if (page == 1) {
                ...
                setAllSchemas(allSchemas.concat(getSmartExtractionSchemasData.smart_extraction_schemas));
            }
    ...
    const search = (label: any) => {
        router.push({ pathname: `/smart_extraction_schema/label/${label?.id}`, query: { label: label.name } })
    }
