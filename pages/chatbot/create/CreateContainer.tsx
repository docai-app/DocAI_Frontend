...
function CreateContainer() {
    ...
    const [{ data: updateChatboxData, loading: updateing }, updateChatbot] = useAxios({}, { manual: true });
    const [chain_feature_ids, set_chain_feature_ids] = useState<any>([])
    const [chatbot, setChatbot] = useState<any>({})
    const [actionContent, setActionContent] = useState('');

    const [
        { data: getChatbotData, loading: loading },
        getChatbot
    ] = useAxios(apiSetting.Chatbot.getChatbotById(router.query.id?.toString() || ''), { manual: true });
