...
export default function HeaderBreadCrumb(props: BreadCrumbProps) {
    const { title, back, next, saveTitle, save } = props;
    return (
        <>
            <div className="max-w-7xl  py-4">
                <div className="flex items-center relative ">
                    ...
                    <label className="text-3xl sm:text-4xl font-bold  flex-1 flex justify-center ">
                        {title}
                    </label>
                    {next && (
                        <label
                    ...
                    {save && <BButton name={saveTitle || '確認'} onClick={save} />}
                </div>
            </div>
        </>
    );
